...
def log_likelihood_ratio_power_vs_exponential(
    ...
    **`smax=None` (既定) は観測最大サイズまで使う = 打ち切らない。** Akita Supplementary は
    「フィットに [1,100] のアバランシェを使った」と書いているが、この 100 を**尤度の評価にも**
    適用すると論文 Fig 2(c) の LLR は再現できない。実測で確定した根拠:

    - **0h は結合ゼロ = 純ポアソンで、パラメータの自由度がゼロ**。よって 0h の LLR が合う実装が
      論文の実装。離散 MLE は −1539(打ち切り) / −950(なし) で論文の ≈−2500 と符号・桁が合う。
      連続版 MLE (`powerlaw` パッケージの既定) は **+724 と符号を誤る**ので除外できる
      (整数サイズを連続扱いすると s=1 の atom で密度が 1 を超えるため)。
    - **6h**: 打ち切りありは 3018、なしは **12822**。論文は **≈12500**。
    → 離散・打ち切りなしだけが 0h と 6h を同時に当てる。

    smax=100 の打ち切りは LLR に天井を与える: [1,100] 上の完全な打ち切りべき乗を生成すると
    指数を 1.1〜2.0 のどこに振っても **1 アバランシェあたり 0.30〜0.48** (最大 α≈1.6 の 0.48)。
    N≈2.4万 なら 1.2e4 が上限で、論文 72h の 2.0e4 (=0.83/アバランシェ) には**べき乗である限り
    届かない**。この天井が「LLR が論文の 1/3」の主因だった。

    打ち切りを外した値は**裾の数個の巨大アバランシェに強く依存する**ので、[1,100] の中だけを
    安定に見たいときは `smax=100` を明示すること (アバランシェ分布図に重ねる理論曲線は
    論文どおり [1,100] で描いており、そちらは別物)。
    """
    smin = 1
