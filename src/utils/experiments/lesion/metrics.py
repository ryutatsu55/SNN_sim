"""損傷実験で 1 probe / 1 時点ぶんの指標をまとめる。

「何を測るか」は実験ごとに違うのでここ (実験モジュール) が持ち、
「どう測るか」は `src/utils/analysis/` の既存関数に任せる。この分担のおかげで、
ここにあるのは呼び出しと列名の組み立てだけになる。
"""
...
from src.utils.experiments.beggs_plenz import analyze_avalanches
...
def spike_metrics(times, ids, layout, num_neurons: int, window_ms: float,
    ...
    `analyze_avalanches` (beggs_plenz) と `split_avalanches` (Akita 系) の**両方**を
...
def cut_profile(coo, cut_mask, layout, num_neurons: int, *, module_axis: str = "module",
    ...
    測る。「切った時点でそれがどういう位置にいたか」が知りたいことなので、切断後に
    測り直したものでは答えにならない。
