...
from scripts.develop.records import SPIKES, discover_records
...
def load_firing_rate_series(run_dir, layout):
    """spikes_*h.npz 群から各時刻のニューロン別発火レートを収集する。

    **npz と config.yaml は別の場所から探す。** run の内部レイアウトでは npz が `data/` に
    あるのに対し config.yaml は run 直下にあり、両方を 1 つのフォルダから読もうとすると
    どちらかを取り逃がす (以前は npz のあるフォルダで config.yaml を探して失敗していた)。
    ...
    records = discover_records(source_dir, SPIKES)

    if not records:
        print(f"警告: フォルダ内に spikes_*h.npz が見つかりません: {source_dir}")
        return None, None, None, None

    config_path = locate(run_dir, CONFIG_NAME)
    if config_path is None:
        print(f"警告: {CONFIG_NAME} が見つかりません: {run_dir}")
        return None, None, None, None
...
def plot_figure2d(folder, layout, output_dir=None):
    ...
    times, rates, exc_ids, inh_ids = load_firing_rate_series(folder, layout=layout)
