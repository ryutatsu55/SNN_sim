...
def plot_figure2c(folder, layout, output_dir=None, metrics_name='metrics.csv',
                  llr_smax=100):
    ...
    Args:
        llr_smax: LLR 天井線を引く打ち切り上限。論文の 100 は **N=100 のこと**なので、
            N が違う run では系のサイズを渡す。`metrics_name` の CSV が
            `avalanche_smax` 列を持つ場合はそちらが優先される。既定 100 は論文条件の
            後方互換。
    """
    ...
    for i, (col, ylabel) in enumerate(metrics_cols):
        ...
        if col in df.columns:
            ...
            if col == 'llr' and 'num_avalanches' in df.columns:
                # べき乗分布が [1, smax] で出せる LLR の上限。論文 Fig2(c) の絶対値は
                # この線を超えており (72h で 1.65 倍)、LLR の絶対値は論文と比較できない。
                # 自分の値が天井の何割かで読むこと。詳細は powerlaw.py の docstring。
                # 注意: metrics の llr は**打ち切らずに**評価した値なので、この線と厳密に
                # 同じ土俵ではない (打ち切り版が要るなら別列を持たせること)。
                ceiling_smax = (int(df['avalanche_smax'].iloc[0])
                                if 'avalanche_smax' in df.columns else int(llr_smax))
                ax.plot(csv_times,
                        llr_ceiling_per_avalanche(smax=ceiling_smax) * df['num_avalanches'],
                        color='tab:green', linestyle='-.', linewidth=1,
                        label=f'power-law ceiling (smax={ceiling_smax})')
