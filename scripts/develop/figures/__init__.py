...
各関数は `(view, out_path)` を受ける。`view` は `scripts/tools/runview.py` の契約
...
def save(fig, out_path, *, tight_layout: bool = True, **savefig_kwargs) -> None:
    ...
    親ディレクトリを作り、保存し、**figure を閉じる**。この 3 つは「忘れると事故になる」
    手順であって見た目の判断ではないので、各図に散らさずここに集めてある。とくに
    `plt.close()` は忘れても動くが、記録窓の数だけ figure が溜まり続ける。

    dpi や bbox_inches は `savefig_kwargs` としてそのまま渡す。**既定値を持たない**のが
    要点で、「どの解像度で出すか」は各図が自分で決める (ここに既定値を置くと、素の
    `plt.savefig()` を使っていた図が黙って差し替わる)。
    """
