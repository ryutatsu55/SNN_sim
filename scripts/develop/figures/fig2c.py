...
重みの軌跡を組み立てる `_weight_trajectories()` は以前 `store/series.py` にあったが、
**唯一の利用者がこの図**なのでここへ移した。読み手に加工を混ぜると、図のファイルを
見ても何を計算しているのか分からなくなる。
"""
...
def fig2c(series, out_path: Path) -> None:
    ...
    if len(df) != len(hours):
        ...
        raise ValueError(
            ...
            " 再解析 (python -m scripts.develop.replot) で作り直してください。"
        )
