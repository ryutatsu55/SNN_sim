"""akita_soc 実験のランチャ (親プロセス)。

    python -m scripts.akita_soc --config <config ファイル名> --condition <条件名>
...
出力の形:
    ...
    outputs/akita_soc/<条件>/            seed が 1 つ  -> ここが run ディレクトリそのもの
    outputs/akita_soc/<条件>/seed01/     seed が複数   -> ここが run ディレクトリ
                        /seed02/
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="akita_soc 実験を走らせる (seed ごとに子プロセスを並列起動)。")
    parser.add_argument(
        "--config", required=True,
        help="config ファイル名。名前だけなら scripts/akita_soc/ の中を見る "
             "(例: akita_soc)。パスを含めれば configs/ のものも指定できる")
    parser.add_argument(
        "--condition", required=True,
        help="条件名。outputs/akita_soc/<条件>/ になる。**条件の記述ではなくラベル** "
             "(何で走ったかは run ディレクトリの config.yaml が持つ)")
    return parser.parse_args()


def resolve_config_path(name: str) -> Path:
    """`akita_soc` / `akita_soc.yaml` / `configs/criticality_test.yaml` を受ける。

    名前だけを渡した場合は **`scripts/akita_soc/` の中**を見る。パスを含む形で渡せば
...
def launch(run_dir: Path) -> int:
    ...
    command = [sys.executable, "-u", "-m", "scripts.akita_soc.run_one", str(run_dir)]
...
def main():
    ...
    if failed:
        ...
        print("  再実行: python -m scripts.akita_soc.run_one <run ディレクトリ>", file=sys.stderr)
        raise SystemExit(1)
