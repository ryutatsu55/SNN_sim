...
## 初期設定
順番にremoteSSHの設定、githubの設定、pythonの環境構築、GeNNのインストール(環境構築時にインストールされる)が必要になる。
...
GeNNについては最新版の5.4を使用している。環境構築のマニュアル通りにやればインストールされるはず。バージョンを変更したかったりする場合は、公式ドキュメントを参考にインストールし直してください。(https://genn-team.github.io/genn/documentation/5/index.html) ※現在はeditable installってやつを使ってて、homeディレクトリにcloneしてインストールされています。
