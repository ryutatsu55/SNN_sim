...
python scripts/akita_soc_fig2.py \
  ...
  --out-dir outputs/akita_soc_72h/run_seed1 \
  > outputs/akita_soc_72h/logs/run_seed1.log 2>&1
```
...
python scripts/akita_soc_fig2.py \
  ...
  --record-buffer-ms 10000 \
  --seed 2 \
  --out-dir outputs/akita_soc_72h/run_seed2 \
  > outputs/akita_soc_72h/logs/run_seed2.log 2>&1
```
...
```bash
tail -f outputs/akita_soc_72h/logs/run_seed1.log
```
...
rclone copy outputs \
  wasedabox:研究室関連/各研究グループ関連/CELL-G/00_進行中プロジェクト/SNN_sim/yamada/ \
  --progress \
  > outputs/akita_soc_72h/logs/rclone_run_seed1.log 2>&1
```
