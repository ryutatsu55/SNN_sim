...
configs/akita_soc.yaml            # --config の既定値
configs/components/tasks.yaml     # タスク akita_soc_fig2 の時間設定
```

ここで区別が必要なのは「configファイル」と「タスク名」です。

- `--config` に渡すのは実行するネットワーク設定ファイルで、既定値は `configs/akita_soc.yaml` です。`akita_soc_delay` や `akita_soc_autapse` など別条件を回す場合は、対応する `configs/akita_soc_*.yaml` を `--config` に指定します。
- スクリプト内部で参照するタスク名は `akita_soc_fig2`（`--task-name` の既定値）で、`configs/components/tasks.yaml` の同名ブロックから記録時刻や記録窓を読み込みます。

現在の `akita_soc_fig2` タスク設定では、シミュレーション内の時間として72時間相当を扱います。
...
なお `configs/components/tasks.yaml` には、より多くの時刻で記録する `akita_soc_fig2_stable_probe`（`record_hours: [0, 1, 3, 6, 12, 24, 48, 72]`）も定義されています。
...
`scripts/akita_soc_fig2.py` が受け付ける主なオプションは次の通りです。tasks.yaml の値は、対応するオプションを渡すと上書きされます。

| オプション | 既定値 | 説明 |
| --- | --- | --- |
| `--config` | `configs/akita_soc.yaml` | 使用するネットワーク設定ファイル |
| `--task-name` | `akita_soc_fig2` | tasks.yaml から読むタスク名 |
| `--duration-hours` | tasks.yaml | 総シミュレーション時間 [h]（実際の進行は主に `--record-hours` の最大値で決まる） |
| `--record-hours` | tasks.yaml | スパイク・重みを記録する時刻リスト [h]（例: `0 6 72`） |
| `--record-hours-range START STOP [STEP]` | なし | 等間隔の記録時刻リストを生成（例: `0 72 1` で1時間刻み73点） |
| `--record-window-ms` | tasks.yaml | 各記録点でスパイクを記録する窓 [ms] |
| `--record-buffer-ms` | tasks.yaml | 記録・進行のチャンク長 [ms] |
| `--seed` | config | 乱数シード（出力ディレクトリ名・GeNNコード生成先に反映） |
| `--out-dir` | なし | 出力のベースディレクトリ（未指定時は `outputs/akita_soc/` 配下に自動生成） |
| `--genn-code-dir` | `genn_code` | GeNN生成コード（`<model名>_CODE`）を集約する親ディレクトリ |
| `--replot-from DIR` | なし | 既存の出力ディレクトリからPNG（ラスター・avalanche・Fig.2c・Fig.2d）とmetricsだけを再生成（シミュレーションは実行しない） |

`--replot-from` は既に完了した実行の `spikes_*h.npz`・`weights_*h.npz`・`metrics.csv`・`config.yaml` を読み込み、ラスター・avalanche図・Figure 2c・Figure 2d と `metrics_replot.csv` を作り直します（本番実行 `main` と同じ可視化一式を再生成）。プロットの体裁だけ変更したい場合に、再シミュレーションせず高速に反映できます。

指定するディレクトリは、`spikes_*h.npz` と `config.yaml` が実際に置かれている場所です。実行完了後は `organize_output` によりこれらが `data/` サブフォルダへ移動しているため、その場合は `--replot-from <run_dir>/data` のように `data/` を指定してください。Figure 2c は `metrics.csv` と `weights_*h.npz`、Figure 2d は `spikes_*h.npz` と `config.yaml` を必要とし、いずれかが欠けている図は警告を出してスキップされます。
...
```bash
sed -n '1,80p' configs/akita_soc.yaml
...
`--out-dir` は実験グループのベースディレクトリとして扱われます。実際の成果物は、その直下に作られる `YYYYmmdd-HHMMSS_seed<N>/` ディレクトリへ保存されます。シードごとにディレクトリ名が分離されるため、同一 config を複数シードで同時に回しても衝突しません。`--out-dir` を省略した場合は、`outputs/akita_soc/` 配下に実行ディレクトリが自動生成されます。
...
本番実行が完了すると、`--out-dir` 直下の `YYYYmmdd-HHMMSS_seed<N>/` に結果が保存されます。最新の実行ディレクトリは次で確認できます。

```bash
RUN_DIR="$(ls -td outputs/akita_soc_72h/*/ | head -n 1)"
ls -lh "$RUN_DIR"
```

実行完了後、`config.yaml`・`*.npz`・`*.csv` は自動で `data/` サブフォルダへ整理されます（`organize_output`）。PNG やその他の可視化はディレクトリ直下に残ります。主な出力は次の通りです。

- `data/config.yaml`: 実行時に解決された設定
- `source_config.yaml`: 元の設定ファイル
- `data/weights_0h.npz` / `data/weights_6h.npz` / `data/weights_72h.npz`: 各記録時刻の重み
- `data/spikes_0h.npz` / `data/spikes_6h.npz` / `data/spikes_72h.npz`: 各記録時刻からの記録窓内スパイク（`times`, `ids`）
- `raster_*.png`: ラスタープロット（興奮性/抑制性のグループ順に並べ替え済み）
- `avalanche_*.png`: avalanche分布プロット
- `data/metrics.csv`: 発火率・臨界性指標（LLR, ΔCR, burstiness, bimodality）に加え、興奮性/抑制性別の発火率や重みブロック統計、活動診断を含む集計値
- `figure2c_reproduction.png`: 論文 Fig.2c 相当（左列=重みの時間発展、右列=ネットワーク指標）の可視化（`plot_figure2c`）
- `figure2d_firing_rate_scatter.png`: 論文 Fig.2d 相当のニューロン別発火レート推移の散布図（興奮性=赤 / 抑制性=青、`plot_figure2d`）
- 重み行列の時間発展トラック（`visualize_weight_tracks`）

Figure 2c / 2d と重みトラックの生成に失敗しても本体処理は続行し、警告が標準出力に表示されます。

`metrics.csv` の先頭を確認します。

```bash
head "$RUN_DIR/data/metrics.csv"
```
