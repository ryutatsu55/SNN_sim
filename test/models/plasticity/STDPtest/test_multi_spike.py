...
本テストは E-STDP と I-STDP の 2 条件を続けて実行する (CONFIGS を参照)。
  - E-STDP: test/models/plasticity/STDPtest/test_multi_estdp.yaml
  - I-STDP: test/models/plasticity/STDPtest/test_multi_istdp.yaml
plasticity.mode を config から自動判別し、対応する素のカーネル
(e_stdp_kernel / i_stdp_kernel) で到着時刻ベースの全ペア和リファレンスを構成する。
=====================================================================

`test/models/plasticity/STDPtest/test.py` は 1 トライアル = pre→post 1 ペアのみを発火させ
...
実行方法:
    ...
    python test/models/plasticity/STDPtest/test_multi_spike.py
"""
